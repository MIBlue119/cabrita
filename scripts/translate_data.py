...
def translate_text(value, target_language):
    response = openai.ChatCompletion.create(
        ...
        messages=[
                ...
                {"role": "user", "content": f"I will talk to you in any language and you will detect the language, translate it and answer with a corrected and improved version of my text, in {target_language}. You could detect which text is programing language or mathmatics. If you encounter any programming languages(Python/Javascript/C++/Swift...etc)/mathematical calculations,ex: `def lcm(a, b):\n  if a > b:\n    greater = a\n  else:\n    greater = b\n  while(True):\n    if((greater % a == 0) and (greater % b == 0)):\n      lcm = greater\n      break\n    greater += 1\n  return lcm\n\nprint(lcm(15, 18))\n# Output: 90`. Please don't translate the code!!! I want you to replace my simplified A0-level words and sentences with more beautiful and elegant high-level {target_language} words and sentences. Keep the same meaning, but make them more literal. I want you to reply only with corrections and improvements, nothing else, no explanation,and don't add any your comment or notes. The text is:\n\n{value}"},
            ],
        max_tokens=1024,
        temperature=0,
        )
